...
class SplashScreenManager {
  ...
  async startFinale() {
    ...
    // Start the official song first, while the entrance click is still the
    // browser's active user gesture. Firework audio is unlocked immediately after.
...
// Lightweight fireworks rendered over the static rectangular Home artwork.
class HomeFireworksManager {
  constructor() {
    this.canvas = document.getElementById('home-fireworks-canvas');
    if (!this.canvas) return;
    this.ctx = this.canvas.getContext('2d');
    this.particles = [];
    this.lowPerformance = document.documentElement.classList.contains('low-performance');
    this.resize();
    window.addEventListener('resize', () => this.resize());
    this.launchBurst();
    this.interval = setInterval(() => {
      if (!document.hidden) this.launchBurst();
    }, this.lowPerformance ? 1400 : 850);
    this.lastFrame = 0;
  ...
  resize() {
    ...
    const dpr = Math.min(window.devicePixelRatio || 1, this.lowPerformance ? 1 : 1.5);
  ...
  launchBurst() {
    const colors = ['#ffd700', '#fff3a8', '#00bfff', '#ffffff'];
    const x = this.width * (0.22 + Math.random() * 0.56);
    const y = this.height * (0.12 + Math.random() * 0.48);
    const color = colors[Math.floor(Math.random() * colors.length)];
    const count = this.lowPerformance ? 16 : 28;
  ...
  render(time) {
    ...
    const frameInterval = this.lowPerformance ? 1000 / 30 : 1000 / 60;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.homeFireworksManager = new HomeFireworksManager();
